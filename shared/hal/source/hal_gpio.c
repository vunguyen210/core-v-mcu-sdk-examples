...
switch(value) {
	case CLEAR:
		if (efpgaio_num < 32)
			efpga->fpgaio_out31_00 &= ~(1U << efpgaio_num) ;
		else if (efpgaio_num < 64)
			efpga->fpgaio_out63_32 &= ~(1U << (efpgaio_num-32)) ;
		else if (efpgaio_num < 80)
			efpga->fpgaio_out79_64 &= ~(1U << (efpgaio_num-64)) ;
		break;
	case SET:
		if (efpgaio_num < 32)
			efpga->fpgaio_out31_00 |= (1U << efpgaio_num) ;
...
void hal_efpgaio_outen(uint8_t efpgaio_num, efpgaio_enum_typedef value) {
	...
	switch(value) {
		case CLEAR:
			if (efpgaio_num < 32)
				efpga->fpgaio_oe31_00 &= ~(1U << efpgaio_num) ;
			else if (efpgaio_num < 64)
				efpga->fpgaio_oe63_32 &= ~(1U << (efpgaio_num-32)) ;
			else if (efpgaio_num < 80)
				efpga->fpgaio_oe79_64 &= ~(1U << (efpgaio_num-64)) ;
			break;
		case SET:
			if (efpgaio_num < 32)
				efpga->fpgaio_oe31_00 |= (1U << efpgaio_num) ;
...
void hal_efpgaio_event(uint8_t event_num) {
	...
	Efpga_t *efpga = (Efpga_t*)EFPGAIO_START_ADDR;
	efpga->fpga_event15_00 |= (1U << event_num);
	efpga->fpga_event15_00 &= ~(1U << event_num);
...
void hal_set_gpio_interrupt(uint8_t gpio_num, uint8_t interrupt_type, uint8_t interrupt_enable){
	...
	regval = regval | (((interrupt_type & REG_SETINT_INTTYPE_MASK) << ( unsigned ) REG_SETINT_INTTYPE_LSB ) |
			 ((interrupt_enable & REG_SETINT_INTENABLE_MASK) << ( unsigned ) REG_SETINT_INTENABLE_LSB) |
			 ((gpio_num & REG_SETINT_gpio_num_MASK ) << ( unsigned ) REG_SETINT_gpio_num_LSB));
...
void hal_enable_gpio_interrupt(uint8_t gpio_num){
	...
	regval = regval & (~(REG_SETINT_gpio_num_MASK << ( unsigned ) REG_SETINT_gpio_num_LSB));
	regval = regval & (~(REG_SETINT_INTENABLE_MASK << ( unsigned ) REG_SETINT_INTENABLE_LSB));
	regval = regval | (((0x1 & REG_SETINT_INTENABLE_MASK) << ( unsigned ) REG_SETINT_INTENABLE_LSB) |
			 ((gpio_num & REG_SETINT_gpio_num_MASK ) << ( unsigned ) REG_SETINT_gpio_num_LSB));
