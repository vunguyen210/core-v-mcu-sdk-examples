...
#include "target/core-v-mcu/include/core-v-mcu-periph.h"
...
#define __FL1(x) (( typeof(x) )31 - ( typeof(x) )__builtin_clz((x)))
						...
						uint32_t *div)
{
	...
	unsigned int D = __MAX(1U, (FLL_LOG2_MAXM - Log2M) >> 1U);
...
uint32_t pi_fll_set_frequency(fll_type_t which_fll, uint32_t frequency, int check)
{
	uint32_t mult, div;
	uint32_t reg1;
...
void pi_fll_init(fll_type_t which_fll, uint32_t ret_state)
{
	...
	if (ret_state) {
		...
		if (freq) {
			...
			flls_frequency[which_fll] = ARCHI_FPGA_FREQUENCY; //freq;
		}
...
uint32_t pi_fll_get_frequency(fll_type_t which_fll, uint8_t real)
{
	if (real) {
