...
#define MQTT_ENDPOINT               "espressif.expresslink.iot.us-west-2.on.aws"
...
int main( void )
{
    ...
    xResult = xTaskCreate( &ConnectTask,
                           "UART",
...
static void i2cCallback( IotI2COperationStatus_t xStatus, void * pvUserContext )
{
    ...
    {
        xSemaphoreGiveFromISR( i2cSemaphore, &switchContexts );
    }
}
...
static void ConnectTask( void * pvParameters )
{
    ...
    received = ExpressLinkCommand( "AT+CONF? ThingName" EOL,
    ...
    for( uint8_t i = 3; i < received - 2; i++ )
    {
        thingName[ i - 3 ] = buffer[ i ];
    }
    thingNameLength = received - 5;
    iot_uart_write_sync( uart0,
                         ( uint8_t * ) "ThingName: ",
                         sizeof( "ThingName: " ) - 1 );
    iot_uart_write_sync( uart0, ( uint8_t * ) thingName, thingNameLength );
    iot_uart_write_sync(
        uart0,
        ( uint8_t * ) "\r\nSending data to visualizer now....... \r\n",
        sizeof( "\r\nSending data to visualizer now....... \r\n" ) - 1 );

    uint32_t lengthWord = ( uint32_t ) snprintf( bufferWord,
                                                 255,
                                                 "AT+CONF Topic1=%.*s/data\r\n",
                                                 thingNameLength,
                                                 thingName );

    received = ExpressLinkCommand( bufferWord,
                                   lengthWord,
                                   buffer,
                                   sizeof( buffer ),
                                   pdMS_TO_TICKS( 1000 ) );
    ...
    for( ;; )
    {
        ...
        lengthWord = snprintf( bufferWord,
                               255,
                               "AT+SEND1 [ { \"label\": \"Room Temperature\", "
                               "\"display_type\" : \"line_graph\", \"values\": "
                               "[ { \"unit\" : \"C\", \"value\" : %d.%02d, "
                               "\"label\" : \" \" } ] } ]\r\n",
                               whole,
                               frac );
        ...
        // vTaskDelayUntil( &xNextWakeTime, pdMS_TO_TICKS( 5000U ) );
    }
}
